...
$selectquery = "select * from studentsscored where name='$_SESSION[username]' AND quiz_attempted='C-Programming'";
...
<div class="main_div">
        ...
        <h3>Bravo! <span class="showname_quiz"><?php echo $_SESSION['username'];?></span>, You have completed the <span class="showname_quiz">C-Programming</span> quiz successfully with a score of</h3>
        <div id="ShowScore" class="ScoreArea">
        <?php 
        $scorecprog=$_SESSION['scorecprog'];
        $_SESSION['scorecprog'] = 0;
        echo $scorecprog;
        $scorecprog=0;
        ...
        <?php
        for($i=1;$i<=10;$i++){
        $q="SELECT * FROM cprogquestions where qid=$i";
        $query=mysqli_query($con,$q);
        $ans_options="SELECT * FROM cproganswers WHERE ans_options=$i AND is_correct=1";
