...
$selectquery = "select * from studentsscored where name='$_SESSION[username]' AND quiz_attempted='JavaScript'";
...
<div class="main_div">
        ...
        <h3>Bravo! <span class="showname_quiz"><?php echo $_SESSION['username'];?></span>, You have completed the <span class="showname_quiz">JavaScript</span> quiz successfully with a score of</h3>
        <div id="ShowScore" class="ScoreArea">
            <?php 
            $scorejavascpt=$_SESSION['scorejavascpt'];
        ...
        <?php
        for($i=1;$i<=10;$i++){
        $q="SELECT * FROM javascptquestions where qid=$i";
        $query=mysqli_query($con,$q);
        $ans_options="SELECT * FROM javascptanswers WHERE ans_options=$i AND is_correct=1";
